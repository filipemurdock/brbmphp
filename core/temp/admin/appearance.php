    ...
    <?php if( ( route(2) == "themes" && !route(3) ) || route(2) != "themes"  ):  ?>
            ...
            <ul class="nav nav-pills nav-stacked p-b">
              <?php foreach($menuList as $menuName => $menuLink ): ?>
                <li class="appearance_menus <?php if( $route["2"] == $menuLink ): echo "active"; endif; ?>"><a class="btn btn-primary margin" href="<?=site_url("admin/appearance/".$menuLink)?>"><?=$menuName?></a></li>
              <?php endforeach; ?>
            </ul>
          </div>
    <?php  endif;
