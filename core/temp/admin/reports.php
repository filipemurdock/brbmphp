    ...
    <div class="mb-4">
        <div class="report-navs__left">
            <ul class="nav nav-tabs p-b">
                <li class="<?php if( $action == "profit" ): echo "active"; endif; ?>">
                    <a class="btn btn-primary margin" href="<?php echo site_url("admin/reports") ?>">Earnings from Orders</a>
                </li>
                <li class="<?php if( $action == "payments" ): echo "active"; endif; ?>">
                    <a class="btn btn-primary margin" href="<?php echo site_url("admin/reports/payments") ?>">Earnings from Payments</a>
                </li>
                <li class="<?php if( $action == "orders" ): echo "active"; endif; ?>">
                    <a class="btn btn-primary margin" href="<?php echo site_url("admin/reports/orders") ?>">Number of Orders</a>
                </li>
            </ul>
        </div>
                ...
                <div class="report-navs__block">
                    <div class="btn-group" role="group">
                        <?php foreach($yearList as $yearl): ?>
                            <a href="<?php echo site_url("admin/reports/".$action."?year=".$yearl) ?>" class="btn btn-primary <?php if( $yearl == $year ): echo "active"; endif; ?>">
                                <?php echo $yearl; ?>
                            </a>
                        <?php endforeach; ?>
                    </div>
                    ...
                    <div class="report-navs__block selectpicker_methods">
                        <select class="selectpicker" data-actions-box="true" data-live-search="true" name="methods[]" multiple="" data-max-options="100" data-size="10" title="Payment Methods" tabindex="-98">
                            <?php foreach($methods as $method ): ?>
                                <option value="<?php echo $method["id"]; ?>" <?php if( $_POST ): if( in_array($method["id"],$_POST["methods"]) ): echo 'selected'; endif; else: echo 'selected'; endif; ?>>
                ...
                <?php if( $action == "profit" || $action == "orders" ): ?>
                                ...
                                <optgroup label="<?=$category?>">
                                    <?php if( !empty($services[0]["service_id"]) ): ?>
                                        <?php for($i=0;$i<count($services);$i++): ?>
                                            <option value="<?php echo $services[$i]["service_id"]; ?>" <?php if( $_POST ): if( in_array($services[$i]["service_id"],$_POST["services"]) ): echo 'selected'; endif; else: echo 'selected'; endif; ?>>
                                                <?php echo $services[$i]["service_id"]." - ".$services[$i]["service_name"] ?>
                                            </option>
                                        <?php endfor; ?>
                                    <?php endif; ?>
                                </optgroup>
                    ...
                    <div class="report-navs__block selectpicker_status">
                        <select class="selectpicker" name="statuses[]" multiple="" data-max-options="100" data-size="10" title="Order Status" tabindex="-98">
                            <option value="cron" <?php if( $_POST ): if( in_array("cron",$_POST["statuses"]) ): echo 'selected'; endif; else: echo 'selected'; endif; ?>>Awaiting Cron</option>
                            <option value="fail"  <?php if( $_POST ): if( in_array("fail",$_POST["statuses"]) ): echo 'selected'; endif; else: echo 'selected'; endif; ?>>Fail</option>
                            <option value="pending"  <?php if( $_POST ): if( in_array("pending",$_POST["statuses"]) ): echo 'selected'; endif; else: echo 'selected'; endif; ?>>Order Received</option>
                    ...
                    <table class="table report-table" style="border:1px solid #ddd">
                        ...
                        <tbody>
                            <?php if( $action == "profit" ): ?>
                                ...
                                <?php endfor; ?>
                                <tr>
                                    <td align="center"><b>Gross Earnings: </b></td>
                                    <?php for( $month=1; $month<=12; $month++ ): ?>
                                        <td align="center">
