  ...
  if( $_POST ){
    ...
    $message  = ($_POST["message"]);
      if( empty($subject) ){
        ...
        $insert3= $insert3->execute(array("c_id"=>$user["client_id"],"action"=>"Nova solicitação de suporte criada#".$ticket_id,"ip"=>GetIP(),"date"=>date("Y-m-d H:i:s") ));
      ...
      if($post){
        ...
        $insert5= $insert5->execute(array("c_id"=>$user["client_id"],"action"=>"Solicitação de suporte <strong>Automatic</strong> answered as. ID:".$ticket_id,"ip"=>GetIP(),"date"=>date("Y-m-d H:i:s") ));
      }
  ...
  if( $_POST ){
   ...
   $message  = ($_POST["message"]);
      if( strlen(str_replace(' ','',$message)) < 5 ){
        ...
        $insert3= $conn->prepare("INSERT INTO client_report SET client_id=:c_id, action=:action, report_ip=:ip, report_date=:date ");
        $insert3= $insert3->execute(array("c_id"=>$user["client_id"],"action"=>"Solicitação de suporte foi respondida #".route(1),"ip"=>GetIP(),"date"=>date("Y-m-d H:i:s") ));
