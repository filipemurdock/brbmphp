...
$data = json_encode($post_vals);
   ...
   <div id="weePay-checkout-form" class="responsive"></div>'; 
    ...
    $amount = $amount * ($extra['dollar_rate'] ?? $dolarrate1); //#sam# configure a taxa aqui
                	       ...
                	       // $users = session('user_current_info');
                            ...
                            $success = 1;
                            $successText = "Seu pagamento foi iniciado com sucesso, você está sendo redirecionado..";
...
curl_setopt_array($curl, [
    CURLOPT_URL => "https://expaybrasil.com/en/purchase/link",
    CURLOPT_RETURNTRANSFER => true,
    CURLOPT_ENCODING => "",
    CURLOPT_MAXREDIRS => 10,
    CURLOPT_TIMEOUT => 30,
    CURLOPT_HTTP_VERSION => CURL_HTTP_VERSION_1_1,
    CURLOPT_CUSTOMREQUEST => "POST",
    CURLOPT_POSTFIELDS => 'merchant_key='.$merchantWebsite.'&currency_code=BRL&invoice={"invoice_id":"'.$ref.'","invoice_description":"Descrição fatura","total":'.$cp_amount.',"devedor":"'.$user['username'].'","email":"'.$u_email.'","cpf_cnpj":"'.$cpf.'","notification_url":"https://'.$dominio.'/notification/index.php","items":[{"name":"name","price":"'.$cp_amount.'.00","description":"description","qty":"1"}]}',
    CURLOPT_HTTPHEADER => [
        "accept: application/json",
...
$insert->execute(array("c_id" => $user['client_id'], "amount" => $cp_amount, "method" => $method_id, "mode" => "Auto", "date" => date("Y.m.d H:i:s"), "ip" => GetIP(), "extra" => $ref));
  ...
  <script>
    document.addEventListener("DOMContentLoaded", function(){
      Swal.fire({
        title: "Escaneie o código QR abaixo para realizar o pagamento...",
        html: `
          <div class="card-box-top" style="color: #2a255c; padding: 20px; border-radius: 10px; margin: 0 0 25px 0; background: rgba(42, 37, 92, 0.15)">
            <img src="data:image/jpg;base64,'.$response->pix_request->pix_code->qrcode_base64.'" id="img-pix" class="img-thumbnail" width="100%" alt="">
          </div>
          ...
          <ul>
            ...
            <li>Após, volte ao site, e seu saldo estará disponível.</li>
          </ul>
          <center>
            <textarea id="codigo-pix" style="width: 100%; height: 70px;" rows="8" class="form-control" name="" rows="8" cols="80">'.$response->pix_request->pix_code->emv.'</textarea>
            <button onclick="copyToClipboard()" style="margin-top:10px;" type="button" class="btn btn-primary" name="button">Copiar Código</button>
            <br><br>
            <button onclick="closeModal()" type="button" class="btn btn-secondary">FECHAR</button>
          </center>
        `,
    ...
    function copyToClipboard() {
      var copyText = document.getElementById("codigo-pix");
      copyText.select();
      document.execCommand("copy");
      Swal.fire({
        title: "Código copiado!",
        text: "Agora você pode colar o código no APP do seu banco.",
        icon: "success",
      });
    }
...
';
  ...
  <script>
    document.addEventListener("DOMContentLoaded", function(){
      Swal.fire({
        title: "Escanei o código QR abaixo para realizar o pagamento...",
        html: `
          <div class="card-box-top" style="color: #2a255c; padding: 20px; border-radius: 10px; margin: 0 0 25px 0; background: rgba(42, 37, 92, 0.15)">
          ...
          <center>
            ...
            <br><br>
            <button onClick="window.location.href=window.location.href" type="button" class="btn btn-secondary" data-bs-dismiss="modal">FECHAR</button>
          </center>
         `,
        customClass: {
