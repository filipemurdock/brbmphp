...
$categories = [];
  foreach ( $categoriesRows as $categoryRow ) {
    ...
    if( $categoryRow["category_secret"] == 2 || $search->rowCount() ):
      $rows = $conn->query("SELECT name_lang, service_name, description_lang, service_description, service_id, service_min, service_max, service_secret, service_image FROM services WHERE category_id='{$categoryRow["category_id"]}' && service_type=2 ORDER BY service_line");
      ...
      $services = [];
       ...
       foreach ( $rows as $row ) {
        ...
        $language   = $language->fetch(PDO::FETCH_ASSOC);
		  ...
		  $s["service_image"] = $row["service_image"]; // Adicione esta linha
