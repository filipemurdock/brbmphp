...
if( $_POST ):
  ...
  $service_detail   = $service_detail->fetch(PDO::FETCH_ASSOC);
    ...
    if( $service_detail["service_package"] == 14 || $service_detail["service_package"] == 15 ){
      ...
      $price    = (service_price($service_detail["service_id"]))*$quantity;
    endif;

    if( $dripfeedon == 1 && $service_detail["service_dripfeed"] == 2):
      $dripfeedon             = 2;
      $dripfeed_totalquantity = $quantity*$runs; //dripfeed toplam gönderim miktarı
      $dripfeed_totalcharges  = service_price($service_detail["service_id"])*$dripfeed_totalquantity; //dripfeed toplam gönderim ücreti
      $price                  = service_price($service_detail["service_id"])*$dripfeed_totalquantity; //dripfeed toplam gönderim ücreti
    else:
      $dripfeedon             = 1;
    ...
    elseif( $service_detail["service_package"] == 1 && ( empty($quantity) ) ):
      $error    = 1;
