...
$payment = $mollie->payments->get($_POST['id']);
  ...
  if ($payment->isPaid() && ! $payment->hasRefunds() && ! $payment->hasChargebacks()){
    ...
    // Atualiza o saldo do cliente
      $update = $conn->prepare('UPDATE payments SET client_balance=:balance, payment_status=:status, payment_delivery=:delivery, payment_extra=:extra WHERE payment_id=:id ');
               $update = $update->execute(['balance' => $payments['balance'], 'status' => 3, 'delivery' => 2, 'extra' => json_encode($extra), 'id' => $payments['payment_id']]);
                $balance = $conn->prepare('UPDATE clients SET balance=:balance WHERE client_id=:id ');
                $balance = $balance->execute(['id' => $payments['client_id'], 'balance' => $user['balance'] + $payments['payment_amount']]);

    // Insere um registro no client_report
    $insertReport = $conn->prepare('INSERT INTO client_report (client_id, action, report_ip, report_date) VALUES (:c_id, :action, :ip, :date)');
    $insertReport->execute(['c_id' => $payment['client_id'], 'action' => 'New ' . $amount . ' ' . $settings["site_currency"] . ' payment has been made with ' . $method['method_name'], 'ip' => GetIP(), 'date' => date('Y-m-d H:i:s')]);
