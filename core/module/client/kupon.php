...
if( !route(1) ){
  ...
  if( $_POST ){
    ...
    $kuponadi  = $_POST["kuponadi"];
      ...
      if( empty($kuponadi) ){
        $error    = 1;
        $errorText= "Prencha o campo por favor.";
      }else{
		...
		if($count["toplam"]>0){
			...
			if($kullanmismi>0){
				 ...
				 $error    = 1;
				$errorText= "Cupom já utilizado anteriormente.";
				...
				$success    = 1;
				$successText= "Cupom resgatado com sucesso!";
			...
			//kullanmismi
				 ...
				 $error    = 1;
				$errorText= "Esse cupom já foi utilizado.";
			}
		  ...
		  $error    = 1;
          $errorText= "Cupom não encontrado.";
		}
