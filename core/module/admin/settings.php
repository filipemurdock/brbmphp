...
$menuList = [
    ...
    "Notificações" => "alert",
    "Bônus" => "payment-bonuses",
    "Moeda" => "currency",
    "Crons" => "crons",
    "Resposta Automática" => "subject"
];
...
$currencies = $conn->prepare("SELECT * FROM currency WHERE nouse=:code");
        ...
        if( route(3) == "add" && $_POST ):
          ...
          if( empty($name) ):
            $error    = 1;
            $errorText= "O nome da moeda não pode ficar vazio";
            $icon     = "error";
          elseif( empty($symbol) ):
            $error    = 1;
            $errorText= "O símbolo da moeda não pode ficar vazio";
            $icon     = "error";
          elseif( empty($value) ):
            $error    = 1;
            $errorText= "A taxa de câmbio não pode ficar vazia";
            $icon     = "error";
          else:
...
elseif (route(2) == "providers"):
    ...
    if ($access):
        ...
        if (route(3) == "new" && $_POST):
            ...
            if (empty($url)):
                $error = 1;
                $errorText = "O URL da API do provedor não pode ficar vazio";
                $icon = "error";
            elseif (empty($key)):
                $error = 1;
    ...
    if ($access):
        if (route(3) == "new" && $_POST):
            ...
            elseif (empty($bank_iban)):
                $error = 1;
                $errorText = "O Digito não pode ficar vazio";
                $icon = "error";
            else:
                $conn->beginTransaction();
        ...
        elseif (route(3) == "edit"):
            ...
            elseif (empty($bank_iban)):
                $error = 1;
                $errorText = "O IBAN não pode ficar vazio";
                $icon = "error";
            else:
                $conn->beginTransaction();
