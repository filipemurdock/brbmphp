...
elseif( $_GET["search_type"] == "domain" && $_GET["search"] && countRow(["table"=>"child_panels","where"=>["panel_domain"=>$_GET["search"]]])  ):
 ...
 $conn->commit();
    ...
    // Verifique se o painel está no status "pending" antes de marcá-lo como "completed".
    if ($panel["panel_status"] == "pending") {
        $conn->beginTransaction();

        $update = $conn->prepare("UPDATE child_panels SET panel_status=:status WHERE id=:id ");
        $update = $update->execute(array("status" => "completed", "id" => $id));

        if ($update) {
            $conn->commit();
            $success = 1;
            $successText = "Painel marcado como 'Concluido' com sucesso.";
        } else {
            $conn->rollBack();
            $error = 1;
            $errorText = "A operação falhou ao marcar o painel como 'Completed'.";
        }
    } else {
        $error = 1;
        $errorText = "O painel não está no status 'pending' e não pode ser marcado como 'concluido'.";
    }
}
