<?php
  ...
  $menuList = ["Temas"=>"themes","Páginas"=>"pages","Notícias"=>"news","Blog"=>"blog","Menu"=>"menu","Idiomas"=>"language","Pastas"=>"files"];
        ...
        if( route(3) == "edit" ):
          ...
          if( $_POST ):
            ...
            if( !countRow(["table"=>"pages","where"=>["page_get"=>$id]]) ):
              $error    = 1;
              $icon     = "error";
              $errorText= "Escolha uma forma de pagamento válida";
            else:
        ...
        if( route(3) == "edit" ):
          ...
          if ( $_FILES["logo"] && ( $_FILES["logo"]["type"] == "image/jpeg" || $_FILES["logo"]["type"] == "image/jpg" || $_FILES["logo"]["type"] == "image/png" || $_FILES["logo"]["type"] == "image/gif"  ) ):
            ...
            $uzanti         = substr($logo_name,-4,4);
                                               $salt = bin2hex(random_bytes(16)); // Gerar um salto aleatório de 16 bytes
    $randomNumber = rand(10, 999);
    $hashedFileName = hash('sha256', $salt . $randomNumber);
    $logo_newname      = "images/painelsam/blog/" . $hashedFileName . ".png";
    ...
    $access = $user["access"]["language"];
      if( $access ):
        ...
        elseif( route(3) == "new" ):
          ...
          include 'core/language/'.route(3).'.php';
         }
        endif;
 ...
 elseif( route(2) == "news" ):
      ...
      if( $access ):
        ...
        if( route(3) == "new" && $_POST ):
          ...
          elseif( empty($content) ):
            ...
            $insert = $insert->execute(array("icon"=>$icon,"title"=>$title,"content"=>$content,"date"=>date("d-m-Y H:i:s") ));
            if( $insert ):
              $conn->commit();
        ...
        elseif( route(3) == "edit" && $_POST  ):
          ...
          $id = route(4);
            if( !countRow(["table"=>"news","where"=>["id"=>$id]]) ):
              $error    = 1;
              $icon     = "error";
              $errorText= "Selecione um anúncio válido";
            else:
    ...
    $randomNumber = rand(10, 999);
    $hashedFileName = hash('sha256', $salt . $randomNumber);
    $logo_newname      = "images/painelsam/" . $hashedFileName . ".png";
            $upload_logo    = move_uploaded_file($_FILES["logo"]["tmp_name"],$logo_newname);
      ...
      if( $access ):
        if( $_POST ):
          ...
          if( $update ):
            ...
            $_SESSION["client"]["data"]["success"] = 1;
            $_SESSION["client"]["data"]["successText"] = "Saved Successfully.";
           else:
            $conn->rollBack();
            $error    = 1;
            $errorText= "Failed";
