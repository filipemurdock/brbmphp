  ...
  elseif( $action == "new-service" ):
      if( $_POST ):
        ...
        elseif( empty($package) ):
          $error    = 1;
          $errorText= "O Pacote do produto não pode estar vazia";
          $icon     = "error";
        ...
        elseif( $min > $max ):
          $error    = 1;
          $errorText= "A quantidade mínima do pedido não pode ser superior à quantidade máxima do pedido";
          $icon     = "error";
        ...
        elseif( empty($secret) ):
          $error    = 1;
          $errorText= "A privacidade do serviço não pode ficar vazia";
          $icon     = "error";
        elseif( empty($want_username) ):
          $error    = 1;
          $errorText= "O link do pedido não pode ficar vazio";
          $icon     = "error";
  ...
  elseif( $action == "new-subscription" ):
      if( $_POST ):
        ...
        elseif(  ( $package == 14 || $package == 15 ) && !is_numeric($autopost) ):
          $error    = 1;
          $errorText= "A quantidade postada não pode estar vazia";
          $icon     = "error";
        ...
        elseif(  ( $package == 14 || $package == 15 ) && !is_numeric($autotime) ):
          $error    = 1;
          $errorText= "A duração do pacote não pode ficar vazia";
          $icon     = "error";
        else:
            $api=$conn->prepare("SELECT * FROM service_api WHERE id=:id "); $api->execute(array("id"=>$provider)); $api=$api->fetch(PDO::FETCH_ASSOC);
            if( $mode == 1 ): $provider = 0; $service = 0; endif;
