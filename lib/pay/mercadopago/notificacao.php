<?php
    ...
    if(isset($_REQUEST['collection_id']) || isset($_REQUEST['id'])){
        ...
        //CONSULTA PAGAMENTO
          ...
          $status = $payments_info["status"];
          $ref    = $payments_info["external_reference"];
        ...
        if($payments){
            ...
            $update = $update->execute(['balance' => $payments['balance'], 'status' => 3, 'delivery' => 2, 'id' => $payments['payment_id']]);

            $balance = $conn->prepare('UPDATE clients SET balance=:balance WHERE client_id=:id ');
            $balance = $balance->execute(['id' => $payments['client_id'], 'balance' => $payments['balance'] + $amount]);
